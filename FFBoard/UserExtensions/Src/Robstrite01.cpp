...
RobStrite_Motor::RobStrite_Motor(uint8_t CAN_Id, CAN_HandleTypeDef *hcan) {
  ...
  _hcan = hcan;
  _can_tx_register = [this](void *header, uint8_t *data) {
    CAN_TxHeaderTypeDef *TxHeader = static_cast<CAN_TxHeaderTypeDef *>(header);
    HAL_CAN_AddTxMessage(_hcan, TxHeader, data, &Mailbox);
    // HAL_CAN_AddTxMessage(_hcan, const CAN_TxHeaderTypeDef *pHeader, const
    // uint8_t *aData, uint32_t *pTxMailbox)
  };
  _delay = [](uint32_t delay) { HAL_Delay(delay); };
}
 ...
 *******************************************************************************/
void RobStrite_Motor::RobStrite_Motor_Analysis(uint8_t *DataFrame,
                                               uint32_t ID_ExtId) {
  if (uint8_t((ID_ExtId & 0xFF00) >> 8) == CAN_ID) {
    // count_num++;
    if (int((ID_ExtId & 0x3F000000) >> 24) == 2) {
      Pos_Info.Angle =
          uint16_to_float(DataFrame[0] << 8 | DataFrame[1], P_MIN, P_MAX, 16);
      Pos_Info.Speed =
          uint16_to_float(DataFrame[2] << 8 | DataFrame[3], V_MIN, V_MAX, 16);
      Pos_Info.Torque =
          uint16_to_float(DataFrame[4] << 8 | DataFrame[5], T_MIN, T_MAX, 16);
...
void RobStrite_Motor::RobStrite_Get_CAN_ID() {
  ...
  TxMessage.DLC = 8;
  TxMessage.ExtId =
      Communication_Type_Get_ID << 24 | Master_CAN_ID << 8 | CAN_ID;
 ...
 * @概述  				: 需要在关闭模式下切换模式
 *******************************************************************************/
void RobStrite_Motor::RobStrite_Motor_move_control(float Torque, float Angle,
                                                   float Speed, float Kp,
                                                   float Kd) {
  uint8_t txdata[8] = {0};       // 发送数据
  CAN_TxHeaderTypeDef TxMessage; // 发送邮箱
  ...
  _can_tx_register(&TxMessage, txdata); // 发送CAN消息
}
/*******************************************************************************
 * @功能     		: RobStrite电机位置模式
 ...
 * @概述  				: 需要在关闭模式下切换模式
 *******************************************************************************/
void RobStrite_Motor::RobStrite_Motor_Pos_control(float Speed,
                                                  float acceleration,
                                                  float Angle) {
  Motor_Set_All.set_speed = Speed;
  Motor_Set_All.set_angle = Angle;
  ...
  // }
  SetCurrentMode(Pos_control_mode);
  Set_RobStrite_Motor_parameter(0X7017, Motor_Set_All.set_speed, Set_parameter);
  Set_RobStrite_Motor_parameter(0X7025, Motor_Set_All.set_acceleration,
                                Set_parameter);
...
void RobStrite_Motor::RobStrite_Motor_Speed_control(float Speed,
                                                    float acceleration,
                                                    float limit_cur) {
  Motor_Set_All.set_speed = Speed;
  ...
  // }
  SetCurrentMode(Speed_control_mode);
  Set_RobStrite_Motor_parameter(0X7018, Motor_Set_All.set_limit_cur,
                                Set_parameter);
  Set_RobStrite_Motor_parameter(0X7022, Motor_Set_All.set_acceleration,
                                Set_parameter);
...
void RobStrite_Motor::RobStrite_Motor_current_control(float current) {
  ...
  SetCurrentMode(Elect_control_mode);
  Set_RobStrite_Motor_parameter(0X7006, Motor_Set_All.set_current,
                                Set_parameter);
}
/*******************************************************************************
 * @功能     		: RobStrite电机零点模式
...
void RobStrite_Motor::Enable_Motor() {
  ...
  TxMessage.DLC = 8;
  TxMessage.ExtId =
      Communication_Type_MotorEnable << 24 | Master_CAN_ID << 8 | CAN_ID;
  _can_tx_register(&TxMessage, txdata); // 发送CAN消息
}
/*******************************************************************************
...
void RobStrite_Motor::Disenable_Motor(uint8_t clear_error) {
  ...
  TxMessage.DLC = 8;
  TxMessage.ExtId =
      Communication_Type_MotorStop << 24 | Master_CAN_ID << 8 | CAN_ID;
 ...
 * @参数1        : 参数地址
 * @参数2        : 参数数值
 * @参数3        : 选择是传入控制模式 还是其他参数 （Set_mode设置控制模式
 *Set_parameter设置参数）
 * @返回值 			: void
 * @概述  				: None
 *******************************************************************************/
void RobStrite_Motor::Set_RobStrite_Motor_parameter(uint16_t Index, float Value,
                                                    char Value_mode) {
  uint8_t txdata[8] = {0};       // 发送数据
  CAN_TxHeaderTypeDef TxMessage; // 发送邮箱
  TxMessage.IDE = CAN_ID_EXT;
  ...
  TxMessage.DLC = 8;
  TxMessage.ExtId =
      Communication_Type_SetSingleParameter << 24 | Master_CAN_ID << 8 | CAN_ID;
  txdata[0] = Index;
  txdata[1] = Index >> 8;
  txdata[2] = 0x00;
  txdata[3] = 0x00;
...
void RobStrite_Motor::Set_CAN_ID(uint8_t Set_CAN_ID) {
  ...
  TxMessage.RTR = CAN_RTR_DATA;
  TxMessage.DLC = 8;
  TxMessage.ExtId = Communication_Type_Can_ID << 24 | Set_CAN_ID << 16 |
                    Master_CAN_ID << 8 | CAN_ID;
  _can_tx_register(&TxMessage, txdata);
  ; // 发送CAN消息
}
/*******************************************************************************
...
void RobStrite_Motor::Set_ZeroPos() {
  ...
  TxMessage.DLC = 8;
  TxMessage.ExtId =
      Communication_Type_SetPosZero << 24 | Master_CAN_ID << 8 | CAN_ID;
  txdata[0] = 1;
  _can_tx_register(&TxMessage, txdata); // 发送CAN消息
  Enable_Motor();
}
/*******************************************************************************
 * @功能     		: RobStrite电机设置当前模式
 * @参数         : 选择电机模式 （0:运控模式 1:位置模式 2:速度模式 3:电流模式
 *4:零点模式)
 * @返回值 			: void
 * @概述  				: None
 *******************************************************************************/
void RobStrite_Motor::SetCurrentMode(uint8_t mode) {
  if (mode > 4) {
    return;
  }
  if (drw.run_mode.data != mode && Pos_Info.pattern == 2) {
    Disenable_Motor(1); // 失能电机
    _delay(20);
    Set_RobStrite_Motor_parameter(0X7005, mode, Set_mode); // 设置电机模式
